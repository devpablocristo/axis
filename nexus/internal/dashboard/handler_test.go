...
func TestDashboardEmpty(t *testing.T) {
	...
	if resp["total_requests"].(float64) != 0 {
		t.Fatalf("expected 0, got %v", resp["total_requests"])
	}
}
...
func TestDashboardWithData(t *testing.T) {
	...
	var resp map[string]any
	json.NewDecoder(rec.Body).Decode(&resp)
	if resp["total_requests"].(float64) != 5 {
		t.Fatalf("expected 5, got %v", resp["total_requests"])
	}
	if resp["allowed"].(float64) != 2 {
		t.Fatalf("expected 2 allowed, got %v", resp["allowed"])
	}
}
