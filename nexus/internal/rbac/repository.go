...
const selectSQL = `SELECT id, org_id, user_id, role, COALESCE(granted_by, ''), granted_at, revoked_at FROM nexus_role_assignments`
...
func (r *PostgresRepository) Create(ctx context.Context, a domain.Assignment) (domain.Assignment, error) {
	...
	_, err := r.db.Pool().Exec(ctx, `
		INSERT INTO nexus_role_assignments (id, org_id, user_id, role, granted_by, granted_at)
		VALUES ($1,$2,$3,$4,$5,$6)
...
func (r *PostgresRepository) Check(ctx context.Context, orgID, userID string, role domain.Role) (bool, error) {
	...
	err := r.db.Pool().QueryRow(ctx, `
		SELECT EXISTS (
			SELECT 1 FROM nexus_role_assignments
...
func (r *PostgresRepository) Archive(ctx context.Context, id uuid.UUID) error {
	...
	if tag.RowsAffected() == 0 {
		...
		if checkErr := r.db.Pool().QueryRow(ctx,
			`SELECT EXISTS (SELECT 1 FROM nexus_role_assignments WHERE id = $1)`, id).Scan(&existed); checkErr != nil {
...
func (r *PostgresRepository) DeleteByID(ctx context.Context, id uuid.UUID) error {
	tag, err := r.db.Pool().Exec(ctx, `DELETE FROM nexus_role_assignments WHERE id = $1`, id)
	if err != nil {
