...
func promptRouteFromRequest(r *http.Request) (promptRoute, error) {
	...
	readProfiles := []string{"companion:virployee_profiles:read", "companion:virployee_profiles:admin", "companion:runtime:admin", "axis:virployees:admin"}
	writeProfiles := []string{"companion:virployee_profiles:admin", "companion:runtime:admin", "axis:virployees:admin"}
	...
	if len(parts) == 1 && parts[0] == "virployee-profiles" && r.Method == http.MethodGet {
		query := r.URL.Query()
		return promptRoute{method: http.MethodGet, path: "/v1/virployee-profiles", rawQuery: query.Encode(), requiredScopes: readProfiles, ok: true}, nil
	}
	if len(parts) == 3 && parts[0] == "virployee-profiles" && parts[2] == "system-prompt" && r.Method == http.MethodPut {
		return promptRoute{
			method:         http.MethodPatch,
			path:           "/v1/virployee-profiles/" + url.PathEscape(parts[1]),
			requiredScopes: writeProfiles,
			forwardBody:    true,
			ok:             true,
		}, nil
	}
	if len(parts) == 3 && parts[0] == "virployee-profiles" && (parts[2] == "archive" || parts[2] == "restore") && r.Method == http.MethodPost {
		return promptRoute{
			method:         http.MethodPost,
			path:           "/v1/virployee-profiles/" + url.PathEscape(parts[1]) + "/" + parts[2],
			requiredScopes: writeProfiles,
			ok:             true,
		}, nil
	}
	if len(parts) == 3 && parts[0] == "virployee-profiles" && parts[2] == "trash" && r.Method == http.MethodPost {
		return promptRoute{
			method:         http.MethodPost,
			path:           "/v1/virployee-profiles/" + url.PathEscape(parts[1]) + "/trash",
			requiredScopes: writeProfiles,
			ok:             true,
		}, nil
	}
	if len(parts) == 3 && parts[0] == "virployee-profiles" && parts[2] == "purge" && r.Method == http.MethodDelete {
		return promptRoute{
			method:         http.MethodDelete,
			path:           "/v1/virployee-profiles/" + url.PathEscape(parts[1]) + "/purge",
			requiredScopes: writeProfiles,
			ok:             true,
		}, nil
	}
