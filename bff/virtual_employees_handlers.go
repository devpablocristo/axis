...
func (s *server) virtualEmployeesDomainAPI(w http.ResponseWriter, r *http.Request) {
	p := principalFromContext(r.Context())
	parts, err := virtualEmployeeRouteParts(r.URL.Path)
	if err != nil {
		writeError(w, http.StatusBadRequest, "VALIDATION", "invalid virtual employee path")
		return
	}
	companionMethod, companionPath, requiredScopes, bodyOverride, ok := virtualEmployeeRoute(r.Method, parts)
	if !ok {
		http.NotFound(w, r)
		return
	}
...
func virtualEmployeeRoute(method string, parts []string) (string, string, []string, string, bool) {
	...
	if len(parts) == 0 {
		switch method {
		case http.MethodGet:
			return http.MethodGet, "/v1/virtual-employees", readScopes, "", true
		case http.MethodPost:
			return http.MethodPost, "/v1/virtual-employees", writeScopes, "", true
		}
	}
	if len(parts) == 1 {
		employeePath := "/v1/virtual-employees/" + url.PathEscape(parts[0])
		switch method {
		case http.MethodGet:
			return http.MethodGet, employeePath, readScopes, "", true
		case http.MethodPatch:
			return http.MethodPatch, employeePath, writeScopes, "", true
		}
	}
	if len(parts) == 2 {
		employeePath := "/v1/virtual-employees/" + url.PathEscape(parts[0])
		...
		case method == http.MethodPost && parts[1] == "status":
			return http.MethodPost, employeePath + "/status", writeScopes, "", true
		case method == http.MethodPost && parts[1] == "archive":
			return http.MethodPost, employeePath + "/status", writeScopes, `{"status":"archived"}`, true
		case method == http.MethodPost && parts[1] == "restore":
			return http.MethodPost, employeePath + "/status", writeScopes, `{"status":"active"}`, true
		case method == http.MethodPost && parts[1] == "trash":
			return http.MethodPost, employeePath + "/status", writeScopes, `{"status":"trashed"}`, true
		}
	}
	return "", "", nil, "", false
}

func virtualEmployeeRouteParts(path string) ([]string, error) {
	path = strings.TrimPrefix(path, "/api/virtual-employees")
	...
	for _, part := range rawParts {
		...
		if strings.TrimSpace(decoded) == "" {
			return nil, fmt.Errorf("empty virtual employee path segment")
		}
