...
CREATE INDEX IF NOT EXISTS idx_agent_profiles_active
	...
	WHERE enabled = true AND archived_at IS NULL AND trashed_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_agent_profiles_trash
	ON agent_profiles (trashed_at)
...
BEGIN
	IF OLD.profile_id IS DISTINCT FROM NEW.profile_id
		...
		OR OLD.trashed_at IS DISTINCT FROM NEW.trashed_at THEN
