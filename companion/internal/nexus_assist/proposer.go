// Package nexus_assist provee asistencia AI sobre el nexus plane.
// Su rol es estricto: leer datos de Nexus y armar artefactos para humanos
// (proposals + summaries). Nunca decide ni ejecuta nexus.
//
// Vive en Companion porque Nexus debe ser AI-independent. Companion lo expone
// vía /companion/v1/nexus-assist/* para que la console de Nexus o un
...
func NewProposer(nexus *nexusclient.Client, llm coreai.Provider) *Proposer {
	return &Proposer{nexus: nexus, llm: llm}
}
...
func (p *Proposer) detectPatterns(ctx context.Context, orgID string, minSampleSize int, minApprovalRate float64) ([]pattern, error) {
	st, raw, err := p.nexus.ListRequests(ctx, fmt.Sprintf("limit=%d", defaultListLimit))
	if err != nil {
		return nil, fmt.Errorf("list requests: %w", err)
	}
	if st >= 400 {
		return nil, fmt.Errorf("list requests: status %d body %s", st, nexusclient.ParseErrorBody(raw))
	}
	var envelope struct {
...
func (p *Proposer) submitToNexus(ctx context.Context, candidate proposalCandidate) error {
	st, raw, err := p.nexus.SubmitProposal(ctx, candidate)
	if err != nil {
		return err
	}
	if st >= 400 {
		return fmt.Errorf("status %d: %s", st, nexusclient.ParseErrorBody(raw))
	}
	return nil
}

const proposerSystemPrompt = `Sos un experto en decisiones sensibles. Analizás patrones de aprobación históricos y proponés políticas CEL para automatizar decisiones repetitivas.
