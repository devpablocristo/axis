...
type Repository interface {
	ListProfiles(ctx context.Context, includeArchived bool) ([]Profile, error)
...
func (u *Usecases) ListProfiles(ctx context.Context, includeArchived bool) ([]Profile, error) {
	return u.repo.ListProfiles(ctx, includeArchived)
}
...
func (u *Usecases) UpsertProfile(ctx context.Context, profile Profile) (Profile, error) {
	...
	if err := validateProfile(profile); err != nil {
		return Profile{}, fmt.Errorf("%w: profile_id, family_id, name, system_prompt and max_autonomy are required", err)
	}
