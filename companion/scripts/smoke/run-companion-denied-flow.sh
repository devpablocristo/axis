...
# Cubre el anti-happy-path del flow companion ↔ nexus.
set -euo pipefail
...
CREATE_BODY=$(companion_post "/v1/tasks" "{\"title\":\"smoke-denied-$(date +%s)\",\"goal\":\"nexus should deny\",\"created_by\":\"smoke-script\"}")
...
STATUS=$(echo "$PROP" | json_get 'nexus_submit.status')
case "$STATUS" in
  denied|rejected)
    pass "Nexus returned $STATUS as expected"
    ;;
  *)
    fail "Expected denied/rejected, got nexus_submit.status=$STATUS"
    ;;
esac
...
echo "Verifying task detail still exposes nexus request linkage..."
if echo "$DETAIL" | python3 -c "
import json, sys
d = json.load(sys.stdin)
sync = d.get('nexus_sync') or {}
ok = bool(sync.get('nexus_request_id')) and bool(sync.get('last_checked_at'))
sys.exit(0 if ok else 1)
"; then
  pass "Task detail still exposes nexus_sync after deny"
else
  fail "Task detail missing nexus_sync after deny"
fi
