...
func TestPostgresRepositoryConcurrentClaimRecoveryAndReplay(t *testing.T) {
	...
	t.Cleanup(func() {
		for _, id := range created {
			_, _ = pool.Exec(context.Background(), `DELETE FROM nexus_jobs WHERE tenant_id=$1 AND id=$2`, tenantID, id)
		}
	})
	...
	if _, err := pool.Exec(ctx, `UPDATE nexus_jobs SET lease_until=now()-interval '1 second' WHERE tenant_id=$1 AND id=$2`, tenantID, deadJob.ID); err != nil {
		t.Fatal(err)
	}
...
func TestPostgresRepositoryWorkerControlAndCircuitBreaker(t *testing.T) {
	...
	t.Cleanup(func() {
		_, _ = pool.Exec(context.Background(), `DELETE FROM nexus_jobs WHERE tenant_id=$1`, tenantID)
		_, _ = pool.Exec(context.Background(), `DELETE FROM nexus_worker_controls WHERE tenant_id=$1`, tenantID)
	})

	if _, err = pool.Exec(ctx, `INSERT INTO nexus_worker_controls(tenant_id,product_surface,kind,state,changed_by,reason_code)VALUES($1,$2,$3,'paused','test','manual_pause')`, tenantID, product, kind); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = pool.Exec(ctx, `UPDATE nexus_worker_controls SET state='closed' WHERE tenant_id=$1 AND product_surface=$2 AND kind=$3`, tenantID, product, kind); err != nil {
		t.Fatal(err)
	}
	for attempt := 0; attempt < 5; attempt++ {
		...
		_, _ = pool.Exec(ctx, `UPDATE nexus_jobs SET run_after=now() WHERE tenant_id=$1 AND status='queued'`, tenantID)
	}
	var state string
	var failures int
	if err = pool.QueryRow(ctx, `SELECT state,failure_count FROM nexus_worker_controls WHERE tenant_id=$1 AND product_surface=$2 AND kind=$3`, tenantID, product, kind).Scan(&state, &failures); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = pool.Exec(ctx, `UPDATE nexus_worker_controls SET opened_until=now()-interval '1 second' WHERE tenant_id=$1 AND product_surface=$2 AND kind=$3`, tenantID, product, kind); err != nil {
		t.Fatal(err)
	}
