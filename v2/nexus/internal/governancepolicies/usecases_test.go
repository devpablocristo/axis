...
func TestCreateVersionRejectsInvalidCELBeforePersistence(t *testing.T) {
	...
	_, err := uc.CreateVersion(context.Background(), "organization-1", "owner-1", "owner", uuid.New(), CreateVersionInput{
...
func TestPromotionRejectsRequesterAndVersionCreator(t *testing.T) {
	...
	repo := &policyRepoStub{
		version:   Version{ID: versionID, OrgID: "organization-1", PolicyID: uuid.New(), State: StateShadow, CreatedBy: "creator-1", ActionTypePattern: "*"},
		promotion: Promotion{ID: promotionID, OrgID: "organization-1", PolicyVersionID: versionID, Status: "pending", RequestedBy: "requester-1"},
	}
	uc := NewUseCases(repo, NewEvaluator(nil), nil)

	if _, err := uc.DecidePromotion(context.Background(), "organization-1", "requester-1", "owner", promotionID, true, PromotionDecisionInput{}); !domainerr.IsForbidden(err) {
		t.Fatalf("requester must not approve own promotion: %v", err)
	}
	if _, err := uc.DecidePromotion(context.Background(), "organization-1", "creator-1", "admin", promotionID, true, PromotionDecisionInput{}); !domainerr.IsForbidden(err) {
