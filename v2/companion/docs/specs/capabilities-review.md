...
| `org_id` | v1 scoped work by org/product context. | Access systems evaluate within a context boundary. | Keep organization-scoped now. |
| `capability_ids` on Virployee | v1 referenced capabilities, not tools. | Roles often collect permissions; here Virployees collect work abilities. | Keep assignment by UUID and validate same organization, active lifecycle and autonomy. |
...
Organization-scoped is correct now because Axis v2 is organized around:

```text
product context = org_id + product_surface
```
