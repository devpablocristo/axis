...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companion_run_traces_virployee_binding
    ON companion_run_traces (tenant_id, virployee_id, binding_hash, created_at DESC)
    WHERE binding_hash <> '';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companion_run_traces_nexus_approval
    ON companion_run_traces (tenant_id, virployee_id, ((nexus_result->>'approval_id')), created_at DESC)
    WHERE nexus_result ? 'approval_id';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companion_run_traces_execution_approval
