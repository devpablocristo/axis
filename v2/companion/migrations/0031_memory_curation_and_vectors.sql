...
CREATE INDEX companion_memories_safe_recall_idx
    ON companion_virployee_memories (tenant_id, virployee_id, updated_at DESC, id DESC)
...
CREATE INDEX companion_memories_embedding_idx
    ON companion_virployee_memories USING hnsw (embedding vector_cosine_ops)
    WHERE embedding IS NOT NULL;

CREATE INDEX companion_memories_decay_idx
    ON companion_virployee_memories (decay_at, id)
    WHERE lifecycle_state = 'active' AND decay_at IS NOT NULL;

ALTER TABLE companion_virployee_memory_audit
    DROP CONSTRAINT IF EXISTS companion_memory_audit_action_check,
    DROP CONSTRAINT IF EXISTS companion_virployee_memory_audit_action_check;
ALTER TABLE companion_virployee_memory_audit ADD CONSTRAINT companion_virployee_memory_audit_action_check
    CHECK (action IN (
        'create', 'update', 'archive', 'unarchive', 'trash', 'restore', 'purge',
        'review_approve', 'review_reject', 'decay', 'index'
    ));

COMMENT ON COLUMN companion_virployee_memories.review_reason IS
    'Machine-readable reason only; never raw memory content, PII, secrets, or model output.';
COMMENT ON COLUMN companion_virployee_memories.poisoning_flags IS
    'Detection categories only; never the matched raw text.';
COMMENT ON COLUMN companion_virployee_memories.pii_flags IS
