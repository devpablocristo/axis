...
func TestRunningJobCancellationIsRequestedThenAcknowledged(t *testing.T) {
	...
	created, _, err := repo.Enqueue(context.Background(), EnqueueInput{TenantID: "tenant", ProductSurface: "companion", Kind: "test", DedupeKey: "cancel-running", Payload: json.RawMessage(`{}`)})
	if err != nil {
		t.Fatal(err)
	}
