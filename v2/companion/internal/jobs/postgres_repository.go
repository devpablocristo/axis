...
func (r *PostgresRepository) Get(ctx context.Context, tenantID string, jobID uuid.UUID) (Job, error) {
	job, err := scanJob(r.pool.QueryRow(ctx, `SELECT `+jobColumns+` FROM companion_jobs WHERE tenant_id=$1 AND id=$2`, strings.TrimSpace(tenantID), jobID))
	if errors.Is(err, pgx.ErrNoRows) {
		return Job{}, ErrJobNotFound
	}
	if err != nil {
...
func (r *PostgresRepository) List(ctx context.Context, tenantID, productSurface, status string, limit int) ([]Job, error) {
	...
	query := `SELECT ` + jobColumns + ` FROM companion_jobs WHERE tenant_id=$1`
