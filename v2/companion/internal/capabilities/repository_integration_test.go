...
func TestRepositoryCapabilityPromotionAndInvalidation(t *testing.T) {
	...
	tenantID := "capability-test-" + uuid.NewString()
	approval := true
	created, err := repository.Create(ctx, tenantID, domain.NormalizedCreateInput{
	...
	updated, err := repository.UpdateManifest(ctx, created.TenantID, created.ID, manifest, hash)
	...
	conformed, err := repository.SaveConformance(ctx, created.TenantID, created.ID, updated, report)
	...
	active, err := repository.Activate(ctx, created.TenantID, created.ID, hash)
	if err != nil || active.PromotionState != domain.PromotionActive || active.ActivatedAt == nil {
		t.Fatalf("Activate: %+v err=%v", active, err)
	}
	changed, err := repository.Update(ctx, created.TenantID, created.ID, domain.NormalizedUpdateInput{
	...
	if _, err := repository.SaveConformance(ctx, created.TenantID, created.ID, active, report); err == nil {
		t.Fatal("stale conformance must not overwrite a concurrent governance change")
	}
	if _, err := repository.Get(ctx, "other-tenant", created.ID); err == nil {
		t.Fatal("capability repository must enforce tenant isolation")
	}
}
