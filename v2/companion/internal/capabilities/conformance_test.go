...
func (c *fakeQuotaPolicyChecker) HasActivePolicies(_ context.Context, tenant, product string, areas []string) (bool, error) {
	c.tenant, c.product, c.areas = tenant, product, append([]string(nil), areas...)
	return c.allowed, nil
}

func validManifestInput() domain.ManifestInput {
	return domain.ManifestInput{
		Version: "1.0.0", ProductSurface: "medmory",
...
func TestCapabilityMustConformBeforeAssignmentAndActivation(t *testing.T) {
	...
	updated, err := uc.UpdateManifest(context.Background(), "tenant-1", id, validManifestInput())
	...
	conformed, report, err := uc.Conform(context.Background(), "tenant-1", id)
	...
	demoted, report, err := uc.Activate(context.Background(), "tenant-1", id)
	...
	active, report, err := uc.Activate(context.Background(), "tenant-1", id)
	...
	if checker.tenant != "tenant-1" || checker.product != "medmory" || len(checker.areas) != 3 {
...
func TestConformanceFailsClosedForMissingPoliciesAndPlainSecret(t *testing.T) {
	...
	capability := domain.Capability{
		TenantID: "tenant-1", SideEffectClass: "write", RequiresNexusApproval: true, EvidenceRequired: true,
...
func TestUpdateManifestRejectsPlainSecretBeforePersistence(t *testing.T) {
	id := uuid.New()
	repo := &fakeCapabilityRepo{rows: map[uuid.UUID]domain.Capability{id: {
		ID: id, TenantID: "tenant-1", CapabilityKey: "calendar.events.create", PromotionState: domain.PromotionDraft,
	}}}
	uc, err := NewUseCases(repo)
	if err != nil {
		t.Fatal(err)
	...
	if _, err := uc.UpdateManifest(context.Background(), "tenant-1", id, input); err == nil {
