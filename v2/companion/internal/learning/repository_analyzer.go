...
// Candidates returns the (virployee, capability) pairs of this organization whose
...
// prepared actions, always organization-scoped.
//
...
func (r *Repository) Candidates(ctx context.Context, orgID string, minExecutions int) ([]Candidate, error) {
	rows, err := r.pool.Query(ctx, `
		...
		WHERE ea.org_id = $1 AND ea.status = 'succeeded'
		GROUP BY 1, 2
		HAVING count(*) >= $2
		ORDER BY 2, 1
	`, orgID, minExecutions)
...
func (r *Repository) LatestForPair(ctx context.Context, orgID string, virployeeID uuid.UUID, capabilityKey string) (*Proposal, error) {
	row := r.pool.QueryRow(ctx, `
		SELECT `+proposalColumns+`
		FROM companion_learning_proposals
		WHERE org_id = $1 AND virployee_id = $2::uuid AND capability_key = $3
		ORDER BY created_at DESC, id DESC
		LIMIT 1
	`, orgID, virployeeID.String(), capabilityKey)
...
func (r *Repository) SuccessfulExecutionTraceIDs(ctx context.Context, orgID string, virployeeID uuid.UUID, capabilityKey string, limit int) ([]string, error) {
	rows, err := r.pool.Query(ctx, `
		SELECT id::text
		...
		LIMIT $4
	`, orgID, virployeeID.String(), capabilityKey, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
