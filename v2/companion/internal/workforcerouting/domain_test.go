...
func TestNormalizeRoutingInputs(t *testing.T) {
	poolID := uuid.New()
	subjectID := uuid.New()
	got, err := NormalizeResolveInput(ResolveInput{PoolID: poolID.String(), SubjectID: subjectID.String(), ActorID: " "})
	if err != nil {
		t.Fatalf("NormalizeResolveInput: %v", err)
	}
	if got.PoolID != poolID || got.SubjectID != subjectID || got.ActorID != "system" {
