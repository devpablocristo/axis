...
export function AgentsControlCenter({ orgId, tenantId, productSurface }: { orgId: string; tenantId: string; productSurface: string }) {
  ...
  const [iamUsers, setIAMUsers] = useState<AxisUserView[]>([])
  const [profilesError, setProfilesError] = useState('')
  const [jobRolesError, setJobRolesError] = useState('')
  const [iamUsersError, setIAMUsersError] = useState('')
  ...
  const supervisorOptions = useMemo(() => iamUsers
    .filter((user) => lifecycleBucket(user.status) === 'active')
    .map((user) => {
      const value = iamUserID(user)
      return value ? { label: formatIAMUserOption(user, value), value } : null
    })
    .filter((option): option is SelectOption => Boolean(option)), [iamUsers])
  ...
  const isActive = tenantId.length > 0 && selectedOrgId.length > 0
  ...
  useEffect(() => {
    if (!tenantId) return
    void loadIAMUserOptions(orgId, tenantId, setIAMUsers, setIAMUsersError)
  }, [orgId, tenantId, reloadVersion, activeSection])
    ...
    >
      ...
      {activeSection === 'agents' ? (
        <CrudPage<AxisAgentView>
          ...
          formFields={agentFormFields(profileOptions, jobRoleOptions, supervisorOptions)}
          ...
          listHeaderInlineSlot={() => (
            <div className="iam-control__lead-stack">
              ...
              {iamUsersError && <p className="iam-control__inline-error">{iamUsersError}</p>}
            </div>
          )}
...
function agentFormFields(
  ...
  return [
    ...
    { key: 'supervisor_user_id', label: 'Supervisor', type: 'select' as const, required: true, options: supervisorOptions, fullWidth: true },
    { key: 'job_role_id', label: 'Job Role', type: 'select' as const, required: true, options: jobRoleOptions, fullWidth: true },
    { key: 'profile_id', label: 'Perfil', type: 'select' as const, required: true, options: profileOptions, fullWidth: true },
...
async function loadIAMUserOptions(
  orgId: string,
  tenantId: string,
  setIAMUsers: (rows: AxisUserView[]) => void,
  setIAMUsersError: (message: string) => void,
) {
  try {
    const users = await listIAMUsers(orgId, 'active', tenantId)
    setIAMUsers(users)
    setIAMUsersError('')
  } catch (err) {
    setIAMUsers([])
    setIAMUsersError(err instanceof Error ? err.message : 'No se pudieron cargar los supervisores')
  }
}
...
function iamUserID(user: AxisUserView): string {
  const explicit = stringValue(user.user_id)
  if (explicit) return explicit
  const rowID = stringValue(user.id)
  if (rowID.startsWith('axis__')) return rowID.slice('axis__'.length)
  if (rowID.startsWith('tenant__')) {
...
function formatIAMUserOption(user: AxisUserView, userID: string): string {
  const email = stringValue(user.email)
  const role = stringValue(user.role)
  const label = email || userID
  return role ? `${label} · ${formatRole(role)}` : label
}
