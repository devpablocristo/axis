...
export function App({ authSlot }: { authSlot?: ReactNode } = {}) {
  ...
  const canViewAdmin = Boolean(canViewControl || session.data?.scopes?.some((scope) => scope === 'companion:capabilities:read' || scope === 'companion:capabilities:admin' || scope === 'companion:runtime:admin'))
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav">
          ...
          {canViewAdmin && <button type="button" className={route.area === 'admin' ? 'active' : ''} onClick={() => navigate({ area: 'admin', screen: 'tools' })}><DatabaseZap aria-hidden="true" />Admin</button>}
      ...
      <main className="workspace">
        ...
        {route.area === 'admin' && canViewAdmin && (
          <AdminControlCenter orgId={orgId} tenantId={tenantId} />
        )}
...
function parseCurrentRoute(): Route {
  ...
  const fromPath = parseRoutePath(window.location.pathname)
  const normalizedPath = routePath(fromPath)
