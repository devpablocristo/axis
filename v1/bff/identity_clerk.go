...
func (a *clerkIdentityAdapter) clerkOrgID(ctx context.Context, axisOrgID string) string {
	...
	if resolved := a.remoteClerkOrgIDByAlias(ctx, axisOrgID); resolved != "" {
		_, _ = a.store.UpdateOrg(ctx, axisOrgID, IAMOrg{Provider: "clerk", ProviderOrgID: resolved})
		return resolved
	}
	return axisOrgID
}

func (a *clerkIdentityAdapter) remoteClerkOrgIDByAlias(ctx context.Context, alias string) string {
	alias = strings.TrimSpace(alias)
	if alias == "" || strings.HasPrefix(alias, "org_") {
		return alias
	}
	const limit = 100
	for offset := 0; ; offset += limit {
		...
		for _, item := range items {
			id := clerkString(item, "id")
			if id == "" {
				continue
			}
			if clerkOrgMatchesAlias(item, alias) {
				return id
			}
		}
...
func clerkOrgMatchesAlias(item map[string]any, alias string) bool {
	...
	candidates := []string{
		clerkString(item, "id"),
		clerkString(item, "name"),
		clerkString(item, "slug"),
	}
	for _, candidate := range candidates {
		normalized := strings.TrimSpace(strings.ToLower(candidate))
		if normalized == alias || normalized == aliasSlug {
			return true
		}
	}
	return false
