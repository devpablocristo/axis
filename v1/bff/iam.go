...
func (s *memoryIAMStore) UpdateOrg(_ context.Context, orgID string, patch IAMOrg) (IAMOrg, error) {
	...
	if strings.TrimSpace(patch.ExternalID) != "" {
		org.ExternalID = strings.TrimSpace(patch.ExternalID)
	}
	if strings.TrimSpace(patch.Provider) != "" {
		org.Provider = strings.TrimSpace(patch.Provider)
	}
	if strings.TrimSpace(patch.ProviderOrgID) != "" {
		org.ProviderOrgID = strings.TrimSpace(patch.ProviderOrgID)
	}
...
func (s *sqlIAMStore) CreateOrg(ctx context.Context, org IAMOrg, actor string) (IAMOrg, error) {
	...
	if err := tx.QueryRowContext(ctx, `
		...
		ON CONFLICT (id) DO UPDATE SET
			external_id = COALESCE(EXCLUDED.external_id, axis_orgs.external_id),
			provider = COALESCE(NULLIF(EXCLUDED.provider, ''), axis_orgs.provider),
			provider_org_id = COALESCE(EXCLUDED.provider_org_id, axis_orgs.provider_org_id),
			name = EXCLUDED.name,
			slug = EXCLUDED.slug,
...
func (s *sqlIAMStore) UpdateOrg(ctx context.Context, orgID string, patch IAMOrg) (IAMOrg, error) {
	...
	err := s.db.QueryRowContext(ctx, `
		UPDATE axis_orgs
		SET external_id = COALESCE(NULLIF($2, ''), external_id),
		    provider = COALESCE(NULLIF($3, ''), provider),
		    provider_org_id = COALESCE(NULLIF($4, ''), provider_org_id),
		...
		WHERE id = $1
		RETURNING id, COALESCE(external_id, ''), provider, COALESCE(provider_org_id, ''), name, slug, status, created_at, updated_at
	`, orgID, patch.ExternalID, patch.Provider, patch.ProviderOrgID, patch.Name, patch.Slug, patch.Status).Scan(&current.ID, &current.ExternalID, &current.Provider, &current.ProviderOrgID, &current.Name, &current.Slug, &current.Status, &current.CreatedAt, &current.UpdatedAt)
