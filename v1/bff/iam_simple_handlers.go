...
func (s *server) iamUsers(w http.ResponseWriter, r *http.Request, parts []string) {
	...
	if r.Method == http.MethodGet && isListRequest(parts) {
		...
		if err := s.syncTenantUsersFromIdentity(r.Context(), tenant); err != nil {
			writeStoreError(w, err)
			return
		}
...
func (s *server) syncTenantUsersFromIdentity(ctx context.Context, tenant IAMTenant) error {
	if s.identity == nil {
		return nil
	}
	if err := s.identity.SyncOrgMembers(ctx, tenant.OrgID); err != nil {
		return err
	}
	members, err := s.iam.ListMembers(ctx, tenant.OrgID)
	if err != nil {
		return err
	}
	for _, member := range members {
		if member.UserID == "" || lifecycleBucket(member.Status) != "active" {
			continue
		}
		...
		if err := s.iam.SetTenantMembership(ctx, tenant.ID, member.UserID, role, "active", platformRoleKeep); err != nil {
			return err
		}
	}
	return nil
