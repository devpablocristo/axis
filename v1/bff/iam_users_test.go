...
func TestTenantUsersListSyncsClerkOrgMembersIntoTenant(t *testing.T) {
	srv, err := newTestServer("http://127.0.0.1:1", defaultAdminScopes())
	if err != nil {
		t.Fatal(err)
	}
	ctx := context.Background()
	if _, err := srv.iam.CreateOrg(ctx, IAMOrg{ID: "cristo.tech", Provider: "clerk", Name: "cristo.tech", Status: "active"}, ""); err != nil {
		t.Fatal(err)
	}
	tenant, err := srv.iam.CreateTenant(ctx, IAMTenant{OrgID: "cristo.tech", ProductSurface: "medmory", Name: "cristo.tech / medmory", Status: "active"})
	if err != nil {
		t.Fatal(err)
	}
	f := newFakeClerk(t, http.StatusOK)
	srv.identity = &clerkIdentityAdapter{
		secretKey:       "sk_test_fake",
		baseURL:         f.server.URL,
		client:          &http.Client{},
		store:           srv.iam,
		productSurfaces: []string{"medmory"},
	}

	rec := doReq(t, srv, http.MethodGet, "/api/iam/users?org_id=cristo.tech", "", map[string]string{"X-Tenant-ID": tenant.ID})
	if rec.Code != http.StatusOK {
		t.Fatalf("list synced users: want 200 got %d body=%s", rec.Code, rec.Body.String())
	}
	var out userListResp
	if err := json.Unmarshal(rec.Body.Bytes(), &out); err != nil {
		t.Fatalf("decode synced users: %v", err)
	}
	user := hasEmail(out.Items, "sync@co-a.com")
	if user == nil {
		t.Fatalf("expected Clerk org member in tenant users, got %#v", out.Items)
	}
	if user.UserID != "user_sync" || user.Role != "admin" || user.TenantID != tenant.ID {
		t.Fatalf("unexpected synced user view: %#v", *user)
	}
	if !f.hit("GET /organizations/org_FAKE/memberships") {
		t.Fatalf("expected list to sync Clerk org memberships; hits=%v", f.paths())
	}
	orgs, err := srv.iam.ListOrgsForActor(ctx, "", true)
	if err != nil {
		t.Fatalf("list orgs after sync: %v", err)
	}
	for _, org := range orgs {
		if org.ID == "cristo.tech" && org.ProviderOrgID != "org_FAKE" {
			t.Fatalf("expected provider_org_id to be cached after Clerk lookup, got %#v", org)
		}
	}
}
